import type { CipheredContent } from '#api/types'
...
export const decipherSecrets = (secrets: Record<string, CipheredContent | string> | undefined, cipherPassword: string): Record<string, string> => {
  const decipheredSecrets: Record<string, string> = {}
  if (!secrets) return decipheredSecrets

  Object.keys(secrets).forEach(key => {
    decipheredSecrets[key] = decipher(secrets[key], cipherPassword)
  })
  return decipheredSecrets
}
