...
# Safe to run from a sandbox (only needs curl and .env).
...
NGINX="http://${DEV_HOST}:${NGINX_PORT}"
...
# --- Docker compose status (if docker available) ---
...
for log in "$PROJECT_DIR"/dev/logs/*.log; do
  ...
  printf "  %-25s %8s bytes  (last modified: %s)\n" "$name" "$size" "$mod"
done
